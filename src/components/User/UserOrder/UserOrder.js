...
import * as AcitonType from "../UserOrder/module/Constants/constants";
...
const UserOrders = () => {
  ...
  const dataDelivered = useSelector(
    (state) => state.reducerOrder.dataDelivered
  );
  ...
  const convertDay = (index) => {
    return moment(index).format("YYYY/MM/DD, h:mm:ss a");
  };
